 ...
 *     pci_subsys_init()
 *      pci_legacy_init()
 *       pcibios_scan_root(busnum==0)
 *        pci_scan_root_bus(parent==NULL, bus=0, ops=pci_root_ops)
 *         pci_scan_child_bus()
 *          pci_scan_child_bus_extend()
 *           pci_scan_single_device() devfn包括了device number和 function number
 *            pci_scan_device()
 *             pci_setup_device()
 *              pci_read_bases(..., howmany==6, rom == PCI_ROM_ADDRESS)
 *               __pci_read_base()
 */
...
static void pci_read_bases(struct pci_dev *dev, unsigned int howmany, int rom)
{
	...
	//读取pci 的base address register的起始地址和大小
...
int pci_setup_device(struct pci_dev *dev)
{
    ...
    //根据设备类型，读取设备的pci 配置信息
	switch (dev->hdr_type) {		    /* header type */
	case PCI_HEADER_TYPE_NORMAL:		    /* standard header */
		if (class == PCI_CLASS_BRIDGE_PCI) //桥设备，不能走这里
			goto bad;

        //读取PCI_INTERRUPT_PIN, PCI_INTERRUTP_LINE信息到dev->irq		
		pci_read_irq(dev);
		//读取bar信息
					      ...
					      unsigned int available_buses)
{
	...
	/* Go find them, Rover!*/
	//bus number ,device number ,function number ,逐个的扫描过去
 ...
 *         pci_scan_child_bus()
 *
 * 扫描总线上的所有设备，如果是pci桥设备，就递归scan下去
 */
 ...
 *       pcibios_scan_root(busnum==0)
 *        pci_scan_root_bus((parent==NULL, bus=0, ops=pci_root_ops)
 *         pci_create_root_bus((parent==NULL, bus=0, ops=pci_root_ops)
 *
 * 创建host bridge
 */
