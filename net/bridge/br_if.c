...
/*
 * sock_ioctl(cmd==SIOCBRADDIF)
 *  sock_do_ioctl(cmd==SIOCBRADDIF)
 *   dev_ioctl(cmd==SIOCBRADDIF)
 *    dev_ifsioc(cmd==SIOCBRADDIF)
 *     br_dev_ioctl(cmd==SIOCBRADDIF)
 *      add_del_if()
 *       br_add_if()
 *        new_nbp()
 *
 ...
 *
 * 创建net_bridge_port对象，关联net_bridge和net_device
 */
	      ...
	      struct netlink_ext_ack *extack)
{
    ...
    //设置dev->rx_handler=br_handle_frame
    ...
    //将这个net_bridge_port链接到net_bridge->port_list上去
	...
	spin_lock_bh(&br->lock);
	//设置该net_bridge->bridge_id.addr==网卡的mac
...
void br_port_flags_change(struct net_bridge_port *p, unsigned long mask)
{
	...
	if (mask & BR_NEIGH_SUPPRESS)
		br_recalcul		b		br_recalculate_neigh_s		br_		br_recalcul		b		br_recalculate_neigh_suppress_enabled(br);
}
}
