			       ...
			       struct sk_buff *skb)
{
	 ...
	 */
	br_vlan_get_tag(skb, &vid); //得到vlan id
...
int nbp_vlan_init(struct net_bridge_port *p)
{
	...
	vg = kzalloc(sizeof(struct net_bridge_vlan_group), GFP_KERNEL);et)
		goto err_rhtbl;
	ret = vlan_tunnel_init(vg);
	if (ret)
		goto err_tunnel_initn_pointer(p->vlgrp, vg);
...
EXPEXPORTEXPEXPORT_SYMBOL_GPLEXEXPEXEEXPEEXPEXPORT_SYMBOL_GPLEXEXPEXEXPEXPORT_SYMEXEXPEXPORT_SYMBOL_GPL(br_vla	p_vinfo->flags = v->flags;
	return 0;
}
EXEXPEXPOREXPEXPORT_SYMBOL_EXEXPEXPORT_SYMBOLEXPEXPORT_SYMBOL_GPLEXEXPEXEXPEXPORT_SYMEXEXPEXPORT_SYMBOL_GPL(br_vlan_get_info);
