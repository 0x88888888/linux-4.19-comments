...
/*
 * sock_ioctl(cmd==SIOCSIFFLAGS) 网桥设备up
 *  sock_do_ioctl(cmd==SIOCSIFFLAGS)
 *   inet_ioctl(cmd==SIOCSIFFLAGS)
 *    devinet_ioctl(cmd==SIOCSIFFLAGS)
 *     dev_change_flags()
 *      __dev_change_flags()
 *       __dev_open()
 */
static int __dev_open(struct net_device *dev)
{
	...
	if (!ret && ops->ndo_open)//br_dev_open
	...
	if (ret)
		...
		dev->flags |= IFF_UP;
		dev_set_rx_mode(dev);
		dev_activate(dev);//active　net_device对象
...
static int __dev_queue_xmit(struct sk_buff *skb, struct net_device *sb_dev)
{
    ...
    // 如果启用了网络优先级 cgroups，这会设置 skb 的优先级skb->priority
	skb_update_prio(skb);
 ...
 *
 * bond_enslave(  )
 *  netdev_rx_handler_register(rx_handler== bond_handle_frame)
 *
 * br_add_if() 重点关注
 *  netdev_rx_handler_register(rx_handler== br_handle_frame)
 *
 * ipvlan_port_create()
 *  netdev_rx_handler_register(rx_handler== ipvlan_handle_frame)
 *
 * ipvtap_newlink()
 *  netdev_rx_handler_register(rx_handler== tap_handle_frame)
 *
 * register_macsec_dev()
 *  netdev_rx_handler_register(rx_handler== macsec_handle_frame)
 *
 * macvlan_port_create()
 *  netdev_rx_handler_register(rx_handler== macvlan_handle_frame)
 *
 * macvtap_newlink()
 *  netdev_rx_handler_register(rx_handler== tap_handle_frame)
 *
 * ovs_netdev_link()
 *  netdev_rx_handler_register(rx_handler== netdev_frame_hook)
 *
 * 设置net_device->rx_handler
 */
...
skip_classify:
	...
	if (rx_handler) {
		...
		case RX_HANDLER_CONSUMED:
			ret = NET_RX_SUCCESS;//被bridge,bond设备处理了
			goto out;
...
int register_netdevice(struct net_device *dev)
{
    ...
    /*
     * 设置net_device->qdisc==noop_disc,这是，这个net_device还不能发送数据
     */
	dev_init_scheduler(dev);
	dev_hold(dev);
	/*
	 * 链接到网络命名空间net->dev_base_head, net->dev_name_head, net->dev_index_head
	 */
	list_netdevice(dev);
	add_device_randomness(dev->dev_addr, dev->addr_len);
 ...
 *
 *
 * 所有的net_device对象都要通过这个函数来将设备挂接到网络名字空间net中去
 */
		...
		unsigned int txqs, unsigned int rxqs)
{
    ...
    //把包含net_device对象的大对象(比如net_bridge)分配在net_device对象的后面,通过netdev_priv(net_device)就可以获取到net_bridge对象了
